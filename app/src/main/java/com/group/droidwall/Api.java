...
public final class Api {
	...
	private static final String PREF_3G_UIDS		= "AllowedUids3G";
	private static final String PREF_WIFI_UIDS	= "AllowedUidsWifi";
	...
	// Messages
	public static final String STATUS_CHANGED_MSG 	= "com.group.droidwall.intent.action.STATUS_CHANGED";
	public static final String TOGGLE_REQUEST_MSG	= "com.group.droidwall.intent.action.TOGGLE_REQUEST";
	public static final String STATUS_EXTRA			= "com.group.droidwall.intent.extra.STATUS";
	...
	public static void showLog(Context ctx) {
		try {
			...
			while ((line = r.readLine()) != null) {
				if (!line.contains("[DROIDWALL]")) continue;
				appid = unknownUID;
	...
	public static DroidApp[] getApps(Context ctx) {
		...
		try {
			...
			@SuppressLint("UseSparseArrays") final HashMap<Integer, DroidApp> map = new HashMap<Integer, DroidApp>();
			final Editor edit = prefs.edit();
			...
			for (final ApplicationInfo apinfo : installed) {
				...
				icon = pkgmanager.getApplicationIcon(apinfo);
				appname = pkgmanager.getApplicationLabel(apinfo).toString();
				version = pkgmanager.getPackageInfo(apinfo.packageName,0).versionName;
				permission = pkgmanager.getPackageInfo(apinfo.packageName,PackageManager.GET_PERMISSIONS).requestedPermissions;
				signature = pkgmanager.getPackageInfo(apinfo.packageName,PackageManager.GET_SIGNATURES).signatures[0].toCharsString();
				if (name.length() == 0) {
			...
			final DroidApp special[] = {
				new DroidApp(SPECIAL_UID_ANY,"(Any application) - Same as selecting all applications",null,false, false),
				new DroidApp(android.os.Process.getUidForName("root"), "(root) - Applications running as root", null, false, false),
				new DroidApp(android.os.Process.getUidForName("media"), "Media server", null, false, false),
				new DroidApp(android.os.Process.getUidForName("vpn"), "VPN networking", null, false, false),
			};
     ...
     */
	private static int runScript(Context ctx, String script, StringBuilder res, long timeout, boolean asroot) {
     ...
     * @return the script exit code
     */
	private static int runScriptAsRoot(Context ctx, String script, StringBuilder res, long timeout) {
     ...
     */
	private static int runScriptAsRoot(Context ctx, String script, StringBuilder res) throws IOException {
	 ...
	 */
	public static void assertBinaries(Context ctx, boolean showErrors) {
		boolean changed = false;
		try {
			// Check iptables_g1
			...
			if (!file.exists()) {
				copyRawFile(ctx, R.raw.busybox_g1, file);
				changed = true;
			}
			if (changed) {
	...
	public static boolean isEnabled(Context ctx) {
		return ctx != null && ctx.getSharedPreferences(PREFS_NAME, 0).getBoolean(PREF_ENABLED, true);
	}
	...
	public static final class DroidApp {
		...
		public DroidApp(int uid, String[] names, Drawable icon, String appname, String version, String[] permission, String
				signature, boolean selected_wifi, boolean selected_3g) {
			this.uid = uid;
			this.names = names;
			this.icon = icon;
		...
		public DroidApp(int uid, String name, Drawable icon, boolean selected_wifi, boolean selected_3g) {
    		this.uid = uid;
